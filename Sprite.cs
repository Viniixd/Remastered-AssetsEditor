...
namespace Assets_Editor
{
    ...
    public class SpriteStorage
    {
        ...
        {
            if (SprLists.TryGetValue((int)id, out MemoryStream cachedStream) && cachedStream != null && cachedStream.Length > 0)
            {
                cachedStream.Position = 0; // Ensure callers always read from the start of the cached PNG stream.
                return cachedStream;
            }

            if (!Sprites.TryGetValue(id, out Sprite sprite))
            {
                return SprLists.TryGetValue(0, out MemoryStream fallback) ? fallback : new MemoryStream();
            }
            ...
            sprite.MemoryStream.SetLength(0); // Clear previous data to avoid appending stale bytes.
            sprite.MemoryStream.Position = 0;
            using (Bitmap bmp = sprite.GetBitmap())
            {
                bmp.Save(sprite.MemoryStream, ImageFormat.Png);
            }
            sprite.MemoryStream.Position = 0; // Guarantee downstream readers consume the PNG from the beginning.
            sprite.CompressedPixels = null;
            SprLists[(int)sprite.ID] = sprite.MemoryStream;

            return sprite.MemoryStream;
        }
